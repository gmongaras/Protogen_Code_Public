...
void change_pixval(uint16_t& pixval, int& y, int& x) {
  ...
  // } else {
  //   // Just use current pixel value
  //   // pixval = bitmap_to_use[cur_frame][y][x];
  // }
  return;
}
...
template <size_t N>
void generate_bitmap_(uint16_t (&bitmap_to_use)[N][HEIGHT][WIDTH2], bool increment, bool is_booping_bm, int cur_frame_) {
  if (is_booping_bm) {
    // Increase the number of frames in boop mode
    num_steps_boop_mode++;
  }
  ...
  for (int y = 0; y < HEIGHT; ++y) {
    for (int x = 0; x < WIDTH2; ++x) {
      ...
      // // Boop mode stuff
      // if (is_booping_bm) {
      //   // Interpolating between normal and boop mode

      //   // Get interpolation proportion
      //   float interp_prop = (float)num_steps_boop_mode / (float)num_iters_boop;

      //   if (pixval == 0x0 && bitmaps_boop[0][y][x] == 0x0) {
      //     float t = constrain(interp_prop, 0.0f, 1.0f);
      //     float pA = sq(sinf(PI * t));              // = sin^2(pi t)

      //     float u =
      //     #ifdef ESP_PLATFORM
      //         (esp_random() * (1.0f / 4294967296.0f));
      //     #else
      //         (random(0L, 1000000L) / 1000000.0f);
      //     #endif
      ...
      //   // // Update intermediate face
      //   float pixval_temp = ((float)(bitmaps_boop[0][y][x]))*interp_prop + ((float)(pixval))*(1-interp_prop);

      //   // // Add noise
      //   // //

      //   // Blend current and boop frames *per channel*
      //   uint16_t blended = lerp565(pixval, bitmaps_boop[0][y][x], interp_prop);

      //   // Add subtle per-channel noise (tune amplitudes)
      //   constexpr int NOISE_R_AMP = 1;   // 0..31
      //   constexpr int NOISE_G_AMP = 1;   // 0..63
      //   constexpr int NOISE_B_AMP = 1;   // 0..31
      //   pixval = add_noise_rgb565(blended, NOISE_R_AMP, NOISE_G_AMP, NOISE_B_AMP);

      //   // Ensure the pixel value is in range
      //   // pixval = (uint16_t)min(max((uint16_t)round(pixval_temp), 0x0000), 0xFFFF);
      // }
...
void generate_bitmap(bool increment=false) {
  ...
  if (is_booping && boop_sensor) {
    // We are in boop mode
    generate_bitmap_<NUM_FRAMES_BOOP>(bitmaps_boop, increment, false, 0);
  } else {
    // We are in normal mode
    generate_bitmap_<NUM_FRAMES>(bitmaps, increment, is_booping, cur_frame);
  }
...
int boopIters = 0;
const int BOOP_NUM_TIERS_TO_CHECK = 10;
...
void loop() {
  ...
  // Is a boop happening?
  boopIters++;
  if (boopIters > BOOP_NUM_TIERS_TO_CHECK) {
    boopIters = 0;
